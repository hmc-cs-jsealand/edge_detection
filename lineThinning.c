char* lineThinning(char* edgePic, int height, int width, bool step) {
	...
	for(int row = 1; row < height - 1; row++) {
		for(int col = 1; col < width - 1; col ++) {
			if(edgePic[row*width + col] == 1) {
				...
				if(step) {
					bool TwoFourSix = (P[2] * P[4] * P[6]) == 0;
					bool FourSixEight = (P[4] * P[6] * P[8]) == 0;
					if( (Ncount > = 2) && (Ncount <= 6) && (Scount == 1) && TwoFourSix && FourSixEight && (P[7] != 0) ) {
				...
				// mark N, W, SE, SW directions
					...
					bool TwoFourEight = (P[2] * P[4] * P[8]) == 0;
					bool TwoSixEight = (P[2] * P[6] * P[8]) == 0;
					if( (Ncount > = 2) && (Ncount <= 6) && (Scount == 1) && TwoFourEight && TwoSixEight && (P[7] != 0) ) {
						thinLinePic[row*width + col] = 1;
						count++;
					}
				}
